...
1. добавил StarRating


# 87 Имплементация оценки для статьи. RTK mutations, queries 
1. реализовал articleRating с помощью rtkQuery


# 88 Алиасы в сторибуке. Приводим сторибук в порядок [!!!ПРОПУЩЕН]
...
# 109 Генерация отчета для скриншотных тестов в CI. Github pages and jobs [!!!ПРОПУЩЕН]

# 110 Отчет для юнит тестов в CI [!!!ПРОПУЩЕН]

# 111 Автофикс для ESLINT правила на относительные пути [!!!ПРОПУЩЕН]

# 112 Исправляем проблемы с зависимостями. Обновляем сторибук 
1. обновил библиотеки 


# 113 Введение в е2е тестирование с Cypress. Тесты на роутер
1. добавил е2е-тесты для профиля и главной страницы


# 114 e2e тесты. Запросы, авторизация. тесты на профиль, статью, комменты, оценку
1. добавил е2е-тесты для страницы статей, страницы статьи с рекомедациями и комментариями


# 115 Моки. Стабы. Фикстуры. Скип тестов. Интерцепторы на запросы и фикстуры
1. добавил fixures и примеры с ними
